...
class CompositeApproachLoss(nn.Module):
    ...
    def forward(self, pred_abs, target_abs, target_pad_mask):
        """
        Compute composite loss combining MSE, ADE and FDE.
        ...
        Returns:
            Scalar loss value (weighted sum of MSE, ADE and FDE losses)
        """
        active_mask = ~target_pad_mask
        loss_mse = self.mse(pred_abs[active_mask], target_abs[active_mask]) if self.weights['mse'] > 0 else 0

        loss_ade = self.ade(pred_abs, target_abs, target_pad_mask) if self.weights['ade'] > 0 else 0
        loss_fde = self.fde(pred_abs, target_abs, target_pad_mask) if self.weights['fde'] > 0 else 0
