...
LOSS_ORDER = ["mse", "ade", "fde", "rtd", "alignment"]
...
class CompositeApproachLoss(nn.Module):
    ...
    def forward(
        ...
        for name in self.enabled_losses:
            if name == "mse":
                active = ~target_pad_mask
                losses.append(self.mse_loss(pred_norm[active], target_norm[active]))
            if name == "ade":
