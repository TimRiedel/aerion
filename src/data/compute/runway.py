...
def get_distances_to_centerline(
    ...
    clamp_m: Optional[float] = 20_000.0,
) -> torch.Tensor:
    """
    Compute (dx, dy) from trajectory positions to each runway centerline point.

    If ``clamp_m`` is not ``None``, values are clamped to ``[-clamp_m, +clamp_m]``
    meters so normalization preserves resolution where the aircraft is close to
    the centerline. If ``clamp_m`` is ``None``, no clamping is applied.
    """
    distances = []
