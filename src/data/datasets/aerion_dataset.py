...
class AerionDataset(ApproachDataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        # Decoder input: shifted positions (3) + shifted deltas (3) + runway-relative coordinates (2)
        dec_in_pos_xy = dec_in_pos[:, :2]
        dec_in_pos_xy_rwy = convert_pos_to_rwy_coordinates(dec_in_pos_xy, threshold_xy, runway_bearing)
        dec_in_traj = torch.cat([dec_in_pos, dec_in_deltas, dec_in_pos_xy_rwy], dim=1)
