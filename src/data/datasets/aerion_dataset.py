...
class AerionDataset(ApproachDataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        # Decoder input: shifted positions (3) + shifted deltas (3) + centerline features (8)
        ...
        dec_in_traj = torch.cat([dec_in_pos, dec_in_deltas, dec_in_dist_runway, dec_in_dist_centerline], dim=1)

        sample = {
            "input_traj": input_traj,            # [T_in, 3 + 3 + 8] positions + deltas + centerline features
            "target_traj": target_traj_deltas,   # [H, 3] target deltas
            "dec_in_traj": dec_in_traj,          # [H, 3 + 3 + 8] decoder input positions + deltas + centerline features
