...
class BaseDataset(Dataset):
    ...
    def get_trajectory_data(self, input_df: pd.DataFrame, horizon_df: pd.DataFrame, flight_id: Optional[str] = None) -> Sample:
        ...
        This method encapsulates the full trajectory pipeline: xyz positions/deltas,
        runway features, feature-schema encoding, and RTD computation. 

        Args:
            input_df: DataFrame with position columns [x_coord, y_coord, altitude].
            horizon_df: DataFrame with position columns for the prediction horizon.
        ...
        xyz_positions, xyz_deltas, target_padding_mask = self._compute_inputs_outputs(input_df, horizon_df)
    ...
    ) -> tuple[TrajectoryData, TrajectoryData, torch.Tensor]:
        """
        Compute xyz positions, xyz deltas, and target padding mask from input and horizon DataFrames.
        """
        horizon_len = len(horizon_df)
